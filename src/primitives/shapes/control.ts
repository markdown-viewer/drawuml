/**
 * Control shape renderer — standalone deployment node.
 *
 * Renders using DrawIO `umlControl` shape with label below the icon,
...
class ControlRenderer extends Renderer {
  ...
  render(box: ContentBox): string[] {
    ...
    let s = `shape=umlControl;verticalLabelPosition=bottom;verticalAlign=top;html=1;outlineConnect=0;`
...
export function registerControlShape(): void {
  registerRenderer('control', (desc: RenderDescriptor) => new ControlRenderer(desc));
}
